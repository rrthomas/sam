...
> `x₁`…`xₙ` `i₁` `s₁` `i₂` `s₂` →
>
> Pop `i₁`, `s₁`, `i₂` and `s₂`. Pop `i₁` stack items, and push them to the stack given by `s₂`, in order from `x₁` to `xₙ`. Push `S0`, `P0`, and `PC` to the stack given by `s₂`. Set `S0` to `s₂`, `P0` to `s₁` and `PC` to `i₂`.
