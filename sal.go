...
func (t *Terminator) String() string {
	...
	} else if t.BreakExp != nil {
		return fmt.Sprintf("break %s", t.BreakExp.String())
	} else if t.Break {
		return "break"
	} else if t.Continue {
		return "continue"
...
func (t *Trap) Compile(ctx *Frame) {
	...
	if stackEffect.Out > 1 {
		panic("Support for traps returning more than one value is not implemented yyet")
	}
	if t.Arguments != nil {
		for _, a := range *t.Arguments {
			a.Compile(ctx)
