...
\begin{table}[htbp]
  \begin{center}
    \begin{tabular}{lrp{2.5in}} \toprule
      ...
      {\tt INVALID\_FUNCTION} & $-12$ & An invalid function number was given to  {\tt TRAP}. \\
      \bottomrule
\end{tabular}
...
\inst{GET}{i -- x}{Pop \spic{i} from the top of the stack. Push the \spic{i}th stack item to the stack. If that item is a stack, push a {\tt LINK} instruction pointing to it.}
\inst{SET}{\x1 i -- }{Pop \spic{i} and \x1 from the stack. Set the \spic{i}th stack item to \x1.}
\inst{IGET}{\I1 \I2 -- x}{Push the \I1th item of the \I2th stack element, which must be a stack or link, to the top of the stack.}
...
\inst{IF}{i \C1 \C2 --}{Pop \C1 and \C2. If either stack item is not code, raise {\tt NOT\_CODE}. Pop \spic{i}. If it is non-zero, perform the action of {\tt DO} on \C1, otherwise on \C2.}
...
\opcodetbl{09}{GET}	{1e}{ADD}
\opcodetbl{0a}{SET}	{1f}{MUL}
\opcodetbl{0b}{IGET}	{20}{DIV}
