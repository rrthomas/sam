...
const sam_word_t SAM_ARRAY_TYPE_MASK = 0x4;
...
sam_word_t sam_run(void)
{
    ...
    for (;;) {
        ...
        sam_uword_t ir;
        HALT_IF_ERROR(sam_stack_peek(sam_pc0, sam_pc++, &ir));
#ifdef SAM_DEBUG
        debug("sam_run: pc0 = %p, pc = %u/%u, sp = %u, ir = %x\n", sam_pc0, sam_pc - 1, sam_pc0->sp, sam_stack->sp, ir);
        sam_print_working_stack(sam_stack);
#endif
        ...
        case SAM_TAG_ATOM:
            ...
            case SAM_ATOM_INST:
            {
                ...
                case INST_POP:
                    ...
                    HALT_IF_ERROR(sam_stack_item(sam_stack, -1, &sam_stack->sp));
                    break;
                case INST_GET:
                    {
                        sam_word_t pos;
                ...
                case INST_SET:
                    {
                        ...
                        sam_uword_t addr;
                        HALT_IF_ERROR(sam_stack_item(sam_stack, pos, &addr));
                        HALT_IF_ERROR(sam_stack_poke(sam_stack, addr, val));
                    }
                    break;
                case INST_IGET:
                    {
                        ...
                        HALT_IF_ERROR(sam_stack_get(s, addr));
                    }
                    break;
                case INST_ISET:
                    {
                        ...
                        POP_WORD(&val);
                        HALT_IF_ERROR(sam_stack_item(s, pos, &addr));
                        HALT_IF_ERROR(sam_stack_poke(s, addr, val));
                    }
                    break;
                case INST_DO:
                    {
        ...
        case SAM_TAG_ARRAY:
        {
            ...
            default:
#ifdef SAM_DEBUG
                debug("*** UNKNOWN ARRAY TYPE %d ***\n", (ir & SAM_ARRAY_TYPE_MASK) >> SAM_ARRAY_TYPE_SHIFT);
#endif
                break;
            }
            PUSH_WORD(ir);
