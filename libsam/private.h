...
#define POP_STACK(addr_var)                     \
    do {                                        \
        sam_uword_t addr;                       \
        _POP_INSN(addr, SAM_TAG_ARRAY, SAM_TAG_MASK, SAM_LINK_MASK, LRSHIFT, 0); \
        addr_var = (sam_stack_t *)addr;         \
    } while(0)

#define POP_FLOAT(var)                                                 \
    do {                                                               \
        sam_uword_t w;                                                 \
        _POP_INSN(w, SAM_TAG_ATOM | (SAM_ATOM_FLOAT << SAM_ATOM_TYPE_SHIFT), SAM_TAG_MASK | SAM_ATOM_TYPE_MASK, SAM_OPERAND_MASK, LRSHIFT, SAM_OPERAND_SHIFT); \
        var = *(sam_float_t *)&w;                                      \
    } while (0)
#define PUSH_FLOAT(val)                                                 \
    do {                                                                \
        sam_float_t v = val;                                            \
        _PUSH_INSN(*(sam_uword_t *)&v, SAM_TAG_ATOM | (SAM_ATOM_FLOAT << SAM_ATOM_TYPE_SHIFT)); \
    } while (0)

// Execution
#define DO(addr)                                \
    do {                                        \
        sam_push_code(sam_stack, sam_pc0);      \
        PUSH_PTR(sam_pc);                       \
        sam_pc0 = (sam_stack_t *)(addr);        \
        sam_pc = 0;                             \
    } while (0)
#define RET                                     \
    do {                                        \
